...
def list_user_projects(request):
    if request.method == "GET":
        ...
        user_projects_data = list(
            user_projects.values(
                "id", "name", "description", "created_at", "updated_at", "auth0_user_id"
            )
        )
...
def create_project(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            project.save()
            return JsonResponse(
                {"status": "success", "project_id": project.id}, status=201
            )
...
def upload_file(request, project_id):
    ...
    if request.method == "POST" and "file" in request.FILES:
        ...
        File.objects.create(project=project, file_name=file.name, file_url=file_url)
...
def get_project_details(request, project_id):  # CONNECTED TO FRONTEND CODE EDITOR
    ...
    if user.get("sub") != project.auth0_user_id:
        return JsonResponse(
            {"status": "error", "message": "This isn't your project"}, status=405
        )
...
class FileViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.info(
                f"File created: {serializer.data['file_name']} for project {project_id}"
            )
            return Response(
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        for file in related_files:
            try:
                ...
                gcs_deletion_errors.append(
                    f"Error deleting {file.file_name} from GCS: {str(e)}"
                )
...
def edit_project_details(request, project_id):
    ...
    temp_user = User.objects.get(
        username="temp_user"
    )  # replace with actual Auth0 check
    ...
    if request.method == "POST":
        if "save_changes" in request.POST:
            ...
            path = default_storage.save(
                "uploads/" + file.name, ContentFile(file.read())
            )
            file_url = default_storage.url(path)
            File.objects.create(project=project, file_name=file.name, file_url=file_url)
    ...
    return render(
        request,
        "api/edit_project_details.html",
        {"project": project, "files": files, "form": form},
    )
