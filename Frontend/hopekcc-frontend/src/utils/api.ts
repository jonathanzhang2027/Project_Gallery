...
export const useAccessToken = () => {
  ...
  return useQuery("accessToken", () => getAccessTokenSilently(), {
...
export const useProjectOperations = (projectId: number) => {
  ...
  const handleProjectRename = async (oldName: string, newName: string) => {
    ...
    const oldProjectData = queryClient.getQueryData<Project>([
      "project",
      projectId,
    ]);
    ...
    try {
      ...
      setError(
        `Failed to rename project from "${oldName}" to "${newName}". The name has been reverted. Please try again.`
      );
...
export const useFileOperations = (projectId: number) => {
  ...
  */
  ...
  const handleFileSave = async (fileId: number, content: string) => {
    const originalFile = queryClient.getQueryData<File>([
      "fileDetails",
      fileId,
    ]);
    if (!originalFile) {
