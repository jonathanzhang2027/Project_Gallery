...
const getFileType = (file: File | undefined): FileType => {
  ...
  // Check for PDF
  if (extension === 'pdf' || file.content.startsWith('%PDF') || file.content.startsWith('JVBERi')) return 'pdf';
  
  // Check for base64 encoded image data
  if (file.content.startsWith('data:image') || file.content.startsWith('iVBORw0KGgo')) return 'img';

  // Check for binary content
  const nonPrintableChars = file.content.match(/[\x00-\x08\x0E-\x1F\x7F-\xFF]/g);
  if (nonPrintableChars !== null && nonPrintableChars.length > file.content.length * 0.1) return 'binary';

  return 'text';
};

const FileTypeMessage: React.FC<{ fileType: FileType, fileName: string }> = ({ fileType, fileName }) => {
  const messages: { [key in FileType]: string } = {
    ...
    unknown: "This file type is unknown and may not be editable in the text editor."
  };
...
export const Editor: React.FC<EditorProps> = React.memo(({ activeFile, onSave, onChange, message }) => {
  ...
  if (fileType !== 'text') {
    return <FileTypeMessage fileType={fileType} fileName={activeFile?.file_name || 'Unknown'} />;
  }
