...
const ProjectEditor: React.FC = () => {
  ...
  const fetchProjectData = async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/project_details/${projectId}/`
      ); // Temporary TODO: USE AUTH TOKEN AS AUTHORIAZSATION HEADER
      ...
      console.log(
        `Fetched project data: ${data.project_name} ${data.project_description}`
      );
    } catch (error) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    const updatedFiles = {
      ...files,
      [activeFile]: { ...files[activeFile], content: e.target.value },
    };
  ...
  return (
    <>
      <ProjectNavBar
        isEditing={isEditing}
        title={title}
        onTitleChange={handleRenameTitle}
