...
export function isValidFileName(fileName: string): {
  ...
  if (trimmedFileName.length > 255) {
    return {
      isValid: false,
      message: "File name is too long. Maximum 255 characters allowed.",
    };
  }
  ...
  if (trimmedFileName.startsWith(".") || trimmedFileName.endsWith(".")) {
    return {
      isValid: false,
      message: "File name cannot start or end with a period.",
    };
  }
  ...
  if (reservedNames.test(trimmedFileName)) {
    return {
      isValid: false,
      message: "This is a reserved file name and cannot be used.",
    };
  }
